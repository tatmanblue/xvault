...
void CDosEnvConfig::Initialize()
{
   if (false == INITIALIZED())
   {
      char *pszWorkDir = getenv("XVAULTWORKDIR");
      char *pszTempDir = getenv("XVAULTTEMPDIR");

      if (NULL != pszWorkDir)
      {
         strncpy(szWorkingDir, pszWorkDir, sizeof(szWorkingDir) - 1);
      }

      if (NULL != pszTempDir)
      {
         strncpy(szTempDir, pszTempDir, sizeof(szTempDir) - 1);
      }
