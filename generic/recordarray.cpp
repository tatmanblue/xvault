...
long CRecordArray::GetNextAvailRecId()
{
   long lRet = 0L;
...
CBaseRecord *CRecordArray::Get(long lRecNo)
{
   if (lRecsInArray <= lRecNo)
   {
      throw new CXVaultException("lRecsInArray <= lRecNo");
   }


   return (pRecords + lRecNo);
}

void CRecordArray::Delete(long)
{
