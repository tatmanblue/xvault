...
CSingleFile::CSingleFile()
{
   ...
   memset(szAssocProgram, 0, sizeof(szAssocProgram));

}

inline const char *CSingleFile::FILENAME() const
{
   return (const char *)szFileName;
}
      
inline const char *CSingleFile::ASSOCPROG() const
{
   return (const char *)szAssocProgram;
}
      
inline long CSingleFile::SIZE() const
{
   return lFileSize;
}
