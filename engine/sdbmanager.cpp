...
CSDBManager::CSDBManager() : fIsInitialized(false), chSDBId(0), pSDBRecords(NULL)
{

}

CSDBManager::CSDBManager(const char chId) : fIsInitialized(false), chSDBId(chId), pSDBRecords(NULL)
{

}

CSDBManager::~CSDBManager()
{
   if (NULL != pSDBRecords)
   {
      delete pSDBRecords;
...
void CSDBManager::Add(const char *pszFile)
{
   if (NULL == pszFile)
   {
      throw new CXVaultException("NULL == pszFile");
   }
}

void CSDBManager::AddFake()
{
